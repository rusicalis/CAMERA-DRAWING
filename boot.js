(() => {
  ...
  function chooseComponent(imageData,gray,w,h,pitchPx,diameter,paperRect) {
    ...
    for(let i=0;i<candidates.length;i++){
      ...
      let roiBonus=0; if(candidates[i].method.includes('촬영판 내부 제한')) roiBonus+=2.4; if(paperRect?.inner){ const pb=candidates[i].pb||{}; const cx=(pb.minX+pb.maxX)/2,cy=(pb.minY+pb.maxY)/2,ir=paperRect.inner,rw=Math.max(1,ir.w),rh=Math.max(1,ir.h),rx=(pb.w||0)/rw,ry=(pb.h||0)/rh,band=Math.max(5,(pitchPx||24)*.55); const touches=[pb.minX<=ir.x0+band,pb.maxX>=ir.x1-band,pb.minY<=ir.y0+band,pb.maxY>=ir.y1-band].filter(Boolean).length; if(cx<ir.x0||cx>ir.x1||cy<ir.y0||cy>ir.y1) roiBonus-=5.0; if(touches>=3)roiBonus-=12; else if(touches===2)roiBonus-=3.0; if(rx>.86&&ry>.86)roiBonus-=10; if(rx>.96||ry>.96)roiBonus-=5; if(rx>.10&&rx<.72&&ry>.18&&ry<.86)roiBonus+=1.2; } candidates[i].finalScore=candidates[i].pathScore+consensus*4.2+bestAgree*1.1+(candidates[i].method.includes('다중 인식 합의')?.65:0)+roiBonus;
    }
  ...
  function majorCenterline(path,diameter=6){
    ...
    const pb=pathBounds(eng),span=Math.max(pb.w,pb.h),minor=Math.min(pb.w,pb.h),plen=pathLength(eng);
    let eps=Math.max((Number(diameter)||6)*0.35,minor*0.010,span*0.0045,1.2);
    if(currentStraightenLevel()==='max') eps*=1.2;
    let pts=rdpSimplify(eng,eps);
    if(pts.length<4) pts=rdpSimplify(eng,Math.max(0.8,eps*0.65));
    pts=collapseMinorKinks(pts,Math.max((Number(diameter)||6)*1.8,plen*0.012),13);
    pts=snapSegmentsToDominantAngles(pts,diameter);
    pts=collapseMinorKinks(pts,Math.max((Number(diameter)||6)*1.4,plen*0.009),9);
    const rb=pathBounds(raw),ob=pathBounds(pts),ol=pathLength(pts),rl=pathLength(raw);
    if(pts.length<3||ob.w<rb.w*.82||ob.h<rb.h*.82||ol<rl*.76||ol>rl*1.18) return eng;
    return pts;
  }
  ...
  function buildCadPath(path, scale=1, diameter=6){
    const pts=majorCenterline(path,diameter); if(pts.length<2) return {points:pts,d:'',arcs:[]};
  ...
  function buildDimensionSpecs(pathMm, pathScreen, W, H, opt={}){
    ...
    const minSeg=Math.max(10,Math.max(mb.w,mb.h)*0.10);
    const segCandidates=[];
    ...
    const segLimit=currentDimensionDetail()==='full'?10:6;
    ...
    const arcsMm=Array.isArray(opt.arcsMm)?opt.arcsMm:[];
    const arcsScreen=Array.isArray(opt.arcsScreen)?opt.arcsScreen:[];
    if(arcsMm.length && arcsScreen.length){
      const limit=currentDimensionDetail()==='full'?8:5;
      for(let k=0;k<Math.min(limit,arcsMm.length,arcsScreen.length);k++){
        const am=arcsMm[k],as=arcsScreen[k];
        if(!(Number.isFinite(am.r)&&am.r>0.5)) continue;
        const v1x=as.p1.x-as.corner.x,v1y=as.p1.y-as.corner.y,v2x=as.p2.x-as.corner.x,v2y=as.p2.y-as.corner.y;
        const m1=Math.hypot(v1x,v1y)||1,m2=Math.hypot(v2x,v2y)||1;
        let nx=v1x/m1+v2x/m2,ny=v1y/m1+v2y/m2; const nm=Math.hypot(nx,ny)||1; nx/=nm; ny/=nm;
        const tx=clamp(as.corner.x+nx*(52+(k%2)*18),60,W-60),ty=clamp(as.corner.y+ny*(52+(k%2)*18),44,H-44);
        specs.push({key:`bendR${k+1}`,value:am.r,kind:'leader',prefix:'R',textX:tx,textY:ty,x1:as.corner.x,y1:as.corner.y,x2:tx-8*nx,y2:ty-8*ny});
        if(currentDimensionDetail()==='full'){
          const a1=Math.atan2(as.corner.y-as.p1.y,as.corner.x-as.p1.x),a2=Math.atan2(as.p2.y-as.corner.y,as.p2.x-as.corner.x);let deg=Math.abs((a2-a1)*180/Math.PI);if(deg>180)deg=360-deg;
          specs.push({key:`bendA${k+1}`,value:deg,kind:'leader',prefix:'A',suffix:'°',textX:clamp(tx+18,60,W-60),textY:clamp(ty+18,44,H-44),x1:as.corner.x,y1:as.corner.y,x2:tx+10,y2:ty+10});
        }
      }
    } else {
      let bendNo=1;
      for(let i=1;i<count-1;i++){
        ...
        bendNo++;
        if(bendNo>(currentDimensionDetail()==='full'?7:4)) break;
      }
    }
  ...
  function buildSvgOnly(r){
    ...
    const mmForDrawing=r.userShapeCorrected?fidelityPath(mmRaw):cadMm.points;
    const mmForDims=mmForDrawing;
    const date=new Date().toLocaleDateString('sv-SE');
    const width=parseFloat(r.width)||r.baseWidth, height=parseFloat(r.height)||r.baseHeight, length=parseFloat(r.length)||pathLength(mmForDrawing);
    let screen=[], shapeMarkup='', photoInset='', dimOpt={};
    if(variant==='photo'){
      ...
      screen=rawScreen;
      shapeMarkup=`<rect x="${draw.x}" y="${draw.y}" width="${draw.w}" height="${draw.h}" fill="none" stroke="#bbb"/>${r.photoDataUrl?`<image href="${r.photoDataUrl}" x="${fit.x}" y="${fit.y}" width="${fit.w}" height="${fit.h}" preserveAspectRatio="xMidYMid meet"/>`:''}<polyline points="${rawScreen.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ')}" fill="none" stroke="rgba(17,17,17,.55)" stroke-width="3"/>`;
    } else {
      ...
      const cadScreen=buildCadPath(screen,1,r.diameter);
  ...
  function makeFigureHtml(r){
    ...
    if(variant==='photo'){
      ...
      const screenBase=mmPath.map(p=>({x:ox+(p.x-pb.minX)*sc,y:oy+(p.y-pb.minY)*sc}));
      const cadScreen=buildCadPath(screenBase,1,r.diameter);
      screen=cadScreen.points;
      dimOpt={arcsMm:cadMm.arcs,arcsScreen:cadScreen.arcs};
    }
    const specs=buildDimensionSpecs(variant==='drawing'?mmPath:transformedPath(r),screen,W,H,dimOpt);
